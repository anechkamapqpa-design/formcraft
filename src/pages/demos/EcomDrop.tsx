    ...
    <div className="min-h-screen bg-[hsl(0,0%,4%)] text-[hsl(0,0%,100%)]">
      ...
      <section className="relative h-screen flex items-center overflow-hidden">
        ...
        <div className="relative z-10 px-8 md:px-16 max-w-2xl">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            <span className="inline-block text-xs font-bold tracking-[0.3em] uppercase bg-[hsl(0,80%,50%)] px-4 py-1.5 mb-6">{d.dropLabel}</span>
            <h2 className="text-6xl md:text-8xl font-black leading-[0.9] mb-4">{d.heroTitle1}<br />{d.heroTitle2}<br />{d.heroTitle3}</h2>
            <p className="text-4xl font-black text-[hsl(0,0%,70%)] mb-8">€149</p>
            <div className="flex gap-3">
              <button className="bg-[hsl(0,0%,100%)] text-[hsl(0,0%,0%)] px-6 py-3 text-sm font-bold uppercase tracking-wider">{d.selectSize}</button>
              <button className="bg-[hsl(0,80%,50%)] px-6 py-3 text-sm font-bold uppercase tracking-wider">{d.buyNow}</button>
            </div>
          </motion.div>
        </div>
      </section>
      <section className="px-6 md:px-16 py-20">
        <div className="flex items-center justify-between mb-10">
          <h3 className="text-2xl font-black uppercase tracking-wider">{d.latestDrops}</h3>
          <a href="#" className="flex items-center gap-1 text-sm text-[hsl(0,0%,50%)] hover:text-[hsl(0,0%,100%)] transition">{d.viewAll} <ChevronRight className="w-4 h-4" /></a>
        </div>
